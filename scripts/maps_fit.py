...
df2014 = df[df["year"] == 2014].rename(
    columns={
        "pv_cap_fit": "pv_cap_2014",
        "pv_inst_fit": "pv_inst_2014",
        "pv_cap_per_inst_fit": "pv_cap_per_inst_2014",
    }
)
df2019 = df[df["year"] == 2019].rename(
    columns={
        "pv_cap_fit": "pv_cap_2019",
        "pv_inst_fit": "pv_inst_2019",
        "pv_cap_per_inst_fit": "pv_cap_per_inst_2019",
    }
)
...
df = df.join(
    df2019[["code", "pv_cap_2019", "pv_inst_2019", "pv_cap_per_inst_2019"]].set_index(
        "code"
    ),
    on="code",
)
...
vmin_cap = df[["pv_cap_2014", "pv_cap_2019"]].min().min()
vmax_cap = df[["pv_cap_2014", "pv_cap_2019"]].max().max()

vmin_inst = df[["pv_inst_2014", "pv_inst_2019"]].min().min()
vmax_inst = df[["pv_inst_2014", "pv_inst_2019"]].max().max()

vmin_ratio = df[["pv_cap_per_inst_2014", "pv_cap_per_inst_2019"]].min().min()
vmax_ratio = df[["pv_cap_per_inst_2014", "pv_cap_per_inst_2019"]].max().max()
...
fig.savefig(
    os.path.join(results_dir, "fig_map_fit.png"),
    dpi=300,
    bbox_inches="tight",
    format="png",
)
