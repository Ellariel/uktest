...
estimates_ordering = {
    ...
    "log_hcapacity": "Households per capita",
    "log_gdp_cap": "GDP per capita",
    "log_fcapacity": "Firms per capita",
    "log_acapacity": "Firm assets per capita",
    "log_income": "Income",
    "log_hholds": "Households",
    "log_sunny_hours": "Sunny hours",
    #
    "W_log_pv_cap": "Spatial term, ρ",
    "W_log_pv_inst": "Spatial term, ρ",
    "W_log_pv_cap_per_inst": "Spatial term, ρ",
    "W_log_pv_cap_fit": "Spatial term, ρ",
    "W_log_pv_inst_fit": "Spatial term, ρ",
    "W_log_pv_cap_per_inst_fit": "Spatial term, ρ",
    "lambda": "Spatial term, λ",
    #
    "W_adensity": "W(D(Firm assets))",
    "W_fdensity": "W(D(Firms))",
    "W_hdensity": "W(D(Households))",
    "W_pdensity": "W(D(Population))",
    "W_log_hcapacity": "W(Households per capita)",
    "W_log_gdp_cap": "W(GDP per capita)",
    "W_log_fcapacity": "W(Firms per capita)",
    "W_log_acapacity": "W(Firm assets per capita)",
    "W_log_income": "W(Income)",
    "W_log_hholds": "W(Households)",
    "W_log_sunny_hours": "W(Sunny hours)",
    "r2": "R2",
    "aic": "AIC",
    "schwarz": "BIC",
    "llr": "Log-Likelihood",
...
def get_full_table(
    ...
    if _results is not None and len(_results) > 0:
        _out = _results.astype(str)
        idx = [i for i in estimates_ordering.keys() if i in _out.index]
        _out = _out.loc[idx]
        _out.index = [estimates_ordering[i] for i in _out.index]
        _cols = _excl(_out.columns, exclude_models)
        _out = _out[_cols]
        _out.to_excel(
    ...
    if _effects is not None and len(_effects) > 0:
        _out = _effects.astype(str)
        idx = [i for i in estimates_ordering.keys() if i in _out.index]
        _out = _out.loc[idx]
        _out.index = [estimates_ordering[i] for i in _out.index]
        _cols = _excl(_out.columns, exclude_models)
        _out = _out[_cols]
        _out.to_excel(os.path.join(res_y, f"{prefix}_effects.xlsx"), engine="openpyxl")
