...
d = df.copy()
d = (
    d[["year", "pv_cap_diff", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
    .pivot(index="year", columns="region", values="pv_cap_diff")
)
...
d = df.copy()
d = (
    d[["year", "pv_inst_diff", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
    .pivot(index="year", columns="region", values="pv_inst_diff")
)
d = d.rename(columns=labels)
...
d = df.copy()
d = (
    d[["year", "pv_cap_per_inst_diff", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
    .pivot(index="year", columns="region", values="pv_cap_per_inst_diff")
)
d = d.rename(columns=labels)
...
fig.savefig(
    os.path.join(results_dir, "fig_dyn_diff.pdf"),
    dpi=1200,
    bbox_inches="tight",
    format="pdf",
)
