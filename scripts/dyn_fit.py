...
d = df.copy()
d = (
    d[["year", "pv_cap_fit", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
    .pivot(index="year", columns="region", values="pv_cap_fit")
)
...
d = df.copy()
d = (
    d[["year", "pv_inst_fit", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
    .pivot(index="year", columns="region", values="pv_inst_fit")
)
d = d.rename(columns=labels)
...
d = df.copy()
d = (
    d[["year", "pv_cap_per_inst_fit", "region"]]
    .groupby(["year", "region"])
    .mean()
    .reset_index()
...
fig.savefig(
    os.path.join(results_dir, "fig_dyn_fit.pdf"),
    dpi=1200,
    bbox_inches="tight",
    format="pdf",
)
